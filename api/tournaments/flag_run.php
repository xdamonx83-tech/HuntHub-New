...
/**
 * /api/tournaments/flag_run.php
 * Flag eines eingereichten Runs (Admin-only).
 */
...
try {
    ...
    $valFlagged = str_contains($type, 'int') ? 3 : 'flagged';

    // Update bauen
    $set = 'status = ?';
    $params = [$valFlagged];

    if (isset($cols['flagged_at'])) $set .= ', flagged_at = NOW()';
    ...
    echo json_encode(['ok'=>true,'id'=>$runId,'status_set'=>$valFlagged]);
} catch (Throwable $e) {
